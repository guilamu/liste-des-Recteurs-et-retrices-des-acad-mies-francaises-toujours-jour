...
async function scrape() {
  ...
  try {
    ...
    // MÉTHODE CORRIGÉE : Extraire depuis le <select class="svg-select">
    console.log("🔎 Extraction des académies depuis le menu déroulant...");
    const linksToVisit = await page.evaluate(() => {
      ...
      return options
        ...
        .map(opt => {
          ...
          if (url.startsWith('/')) {
            url = 'https://www.education.gouv.fr' + url;
          }
        ...
        .filter(item => item.name !== 'Choisir une académie' && item.name !== '');
    });

    console.log(`✅ ${linksToVisit.length} académies trouvées.`);

    if (linksToVisit.length > 0) {
      console.log("\n📋 Liste des académies détectées :");
      linksToVisit.forEach((item, i) => {
    ...
    if (linksToVisit.length === 0) {
      console.log("❌ ERREUR : Aucune académie trouvée dans le select.");
